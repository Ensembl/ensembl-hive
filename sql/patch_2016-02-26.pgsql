...
-- going back to the ENUM since it is much more efficiently indexed:
...
DROP VIEW progress;     -- it was in the way, blocking the status column

ALTER TABLE  job  ALTER COLUMN  status  DROP DEFAULT;
ALTER TABLE  job  ALTER COLUMN  status  TYPE  job_status USING CAST(status AS job_status);
ALTER TABLE  job  ALTER COLUMN  status  SET DEFAULT CAST('READY' AS job_status);

-- recreating the progress view again:

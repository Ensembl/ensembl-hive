...
sub main {
    ...
    if(@$tweaks) {
        my ($need_write, $msg_list_ref, $responce_structure) = $pipeline->apply_tweaks( $tweaks );

        $responce_structure->{URL} = $self->{'url'};
        my $json = JSON->new->allow_nonref;

        print $self->{'json'} ? $json->encode($responce_structure) : join('', @$msg_list_ref);
        if ($need_write) {

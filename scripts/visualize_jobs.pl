...
sub add_job_node {
    ...
    my $job_node_name   = 'job_'.$job->dbID;

    unless($job_node_hash{$job_node_name}++) {
        my $job_shape           = 'Mrecord';
        my $job_status_colour   = {'DONE' => 'DeepSkyBlue', 'READY' => 'green', 'SEMAPHORED' => 'orange', 'FAILED' => 'red'}->{$job->status} // 'blue';
        my $job_id              = $job->dbID;
        my $job_params          = destringify($job->input_id);

        my $job_label           = qq{<<table border="0" cellborder="0" cellspacing="0" cellpadding="1">}
                                 .qq{<tr><td bgcolor="grey">job_id:</td><td bgcolor="grey">$job_id</td></tr>};
        while(my ($param_key, $param_value) = each %$job_params) {
        ...
        $self->{'graph'}->add_node( $job_node_name,
            shape       => 'record',
            comment     => qq{new_shape:$job_shape},
            style       => 'filled',
...
sub add_semaphore_node {
    ...
    my $semaphore_label             = $semaphore->local_jobs_counter .' + '. $semaphore->remote_jobs_counter;
    my $semaphore_status_colour     = ($semaphore_label eq '0 + 0') ? 'darkgreen' : 'purple';
    my $dependent_blocking_colour   = ($semaphore_label eq '0 + 0') ? 'grey' : 'red';

    unless($semaphore_node_hash{$semaphore_node_name}++) {
        $self->{'graph'}->add_node( $semaphore_node_name,
            shape       => 'trapezium',
            style       => 'filled',
            fillcolor   => $semaphore_status_colour,
        ...
        if(my $dependent_job = $semaphore->dependent_job) {
            ...
            $self->{'graph'}->add_edge( $semaphore_node_name => $dependent_job_node_name,
                color       => $dependent_blocking_colour,
                style       => 'dashed',
                arrowhead   => 'tee',
            );
...
sub add_family_tree {
    ...
    if(my $controlled_semaphore = $parent_job->controlled_semaphore) {
        ...
        my $parent_controlling_colour   = ($parent_status eq 'DONE' or $parent_status eq 'PASSED_ON') ? 'grey' : 'red';

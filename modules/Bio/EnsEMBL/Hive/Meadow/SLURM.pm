...
sub get_report_entries_for_process_ids {
    ...
    while (my $pid_batch = join(',', map { "'$_'" } splice(@_, 0, 20))) {  # can't fit too many pids on one shell cmdline
     ...
     #$pid_batch =~ s/\]//g;
        ...
        my $cmd = "sacct -n -p --units=M --format JobName,JobID,ExitCode,MaxRSS,MaxDiskRead,CPUTimeRAW,ElapsedRAW,State,DerivedExitCode,End -j $pid_batch  " ;
...
sub get_report_entries_for_time_interval {
    ...
    my $cmd = "sacct -n -p --units=M -s CA,CD,F,OOM --format JobName,JobID,ExitCode,MaxRSS,MaxDiskRead,CPUTimeRAW,ElapsedRAW,State,DerivedExitCode,End -S $from_time -E $to_time ".($username ? "-u $username" : '') . ' |';
...
sub execute_command {
  ...
  if ($success) {
     ...
     die "Failed to execute command '$cmd'. Error: '$error_message', STDOUT: '@$stdout_buf', STDERR: '@$stderr_buf'";
  }
}

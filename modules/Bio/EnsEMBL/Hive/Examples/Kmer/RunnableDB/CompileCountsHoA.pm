...
=head1 SYNOPSIS
    ...
    Please refer to Bio::EnsEMBL::Hive::Examples::Kmer::PipeConfig::KmerPipelineHoA_conf pipeline configuration file
...
package Bio::EnsEMBL::Hive::Examples::Kmer::RunnableDB::CompileCountsHoA;
...
=head2 run
    ...
    This method expects counts to be stored in the accumulator as an hash of arrays. The key in the hash is the kmer sequence, and the value
    is the list of counts of this kmer in the chunk files (e.g. [5, 8, ...]). Each element of the array is a value generated by a CountKmers
    job run previously in this pipeline.
...
sub run {
    ...
    # Loop through all the keys stored in the accumulator.
    foreach my $kmer (keys %{$all_counts}) {

      # for each kmer, retrieve the counts for each particular CountKmers job, and add to our total.
      foreach my $c (@{$all_counts->{$kmer}}) {
	  $sum_of_counts{$kmer} += $c;
      }
    }

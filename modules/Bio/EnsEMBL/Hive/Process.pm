...
sub life_cycle {
    ...
    if(my $life_cycle_msg = $@) {
        ...
        Bio::EnsEMBL::Hive::Process::warning($self, $life_cycle_msg, $job->incomplete);     # In case the Runnable has redefined warning()
    }
    ...
    if( $self->can('post_cleanup') ) {   # may be run to clean up memory even after partially failed attempts
        ...
        if(my $post_cleanup_msg = $@) {
            $job->died_somewhere( $job->incomplete );  # it will be OR'd inside
            Bio::EnsEMBL::Hive::Process::warning($self, $post_cleanup_msg, $job->incomplete);   # In case the Runnable has redefined warning()
        }
    }

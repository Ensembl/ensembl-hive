...
sub hash_to_url {
  ...
  my $params_part = '';
  if ($parse->{'query_params'}) {
    my $object_type = $parse->{'query_params'}->{'object_type'};
    
    if ($object_type eq 'Analysis') {
      if ($driver_part ne '') {
	$params_part = '?logic_name=';
      }
      $params_part .= $parse->{'query_params'}->{'logic_name'};
      
    } elsif ($object_type eq 'NakedTable') {
      $params_part .= '?table_name=' . $parse->{'query_params'}->{'table_name'};
      
    } elsif ($object_type eq 'Accumulator') {
      $params_part = '?accu_name=' .
	$parse->{'query_params'}->{'accu_name'} .
	  '&accu_address=' .
	    $parse->{'query_params'}->{'accu_address'} .
	      '&accu_input_variable=' .
		$parse->{'query_params'}->{'accu_input_variable'};
      
    } else {
      die "Reconstructiong URLs for and object of type \"$object_type\" is not yet supported";
    }
  }

  my $url = $driver_part . $params_part;

...
Bio::EnsEMBL::Hive::RunnableDB::LoadIntoTable2

=head1 SYNOPSIS

    standaloneJob.pl Bio::EnsEMBL::Hive::RunnableDB::LoadIntoTable2 --table meta_foo \
...
package Bio::EnsEMBL::Hive::RunnableDB::LoadIntoTable2;
...
sub run {
    ...
    my $select_sth = $src_dbc->prepare($self->param_required('inputquery'));
    ...
    my $select_n   = 0;
    my $insert_n   = 0;
    my $next_row   = $select_sth->fetch();
    while ($next_row) {
        my $insert_sql = sprintf('%s INTO %s (%s) VALUES ', $insert_cmd, $table, join(',', @$col_names));
        my $first = 1;
        while ($next_row and (length($insert_sql) < 1_000_000)) {
            $select_n ++;
            $insert_sql .= ($first ? '' : ', ') . '(' . join(',', map {'"'.$_.'"'} @$next_row) . ')';
            $first = 0;
            $next_row = $select_sth->fetch();
        }
        $insert_n += $dest_dbc->do($insert_sql);
    }

...
sub apply_tweaks {
    ...
    foreach my $tweak (@$tweaks) {
        ...
        if($tweak=~/^pipeline\.param\[(\w+)\](\?|#|=(.+))$/) {
            ...
            if($operator eq '?') {
                my $value = $hash_pair ? $hash_pair->{'param_value'} : '(missing_value)';
          ...
          push @{$responceStructure->{Tweaks}}, $tweakStructure;
    ...
    return $need_write, \@responce, $responceStructure;
}
